...
async function main() {
  ...
  try {
    ...
    try {
      ...
      console.log("   - Successful Trades:", existingBot.successfulTrades.toString());
      return;
    } catch (error) {
    ...
    console.log("   - Created At:", new Date(bot.createdAt.toNumber() * 1000).toLocaleString());
    console.log("   - Strategy Type:", Object.keys(bot.strategy.strategyType)[0]);
    ...
    const pauseTx = await program.methods
      ...
      .rpc();
    console.log("✅ Bot paused:", `https://explorer.solana.com/tx/${pauseTx}?cluster=devnet`);
    ...
    const resumeTx = await program.methods
      ...
      .rpc();
    console.log("✅ Bot resumed:", `https://explorer.solana.com/tx/${resumeTx}?cluster=devnet`);
    ...
    const priceUpdateTx = await program.methods
      ...
      .rpc();
    console.log("✅ Price updated:", `https://explorer.solana.com/tx/${priceUpdateTx}?cluster=devnet`);
