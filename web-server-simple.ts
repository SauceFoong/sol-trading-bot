...
class WebSocketMonitor {
  ...
  constructor(server: http.Server) {
    ...
    this.provider = anchor.AnchorProvider.local(rpcUrl)
    anchor.setProvider(this.provider)
    
    this.program = anchor.workspace.SolanaTradingBot
  ...
  private async addBotToMonitoring(botAccountPubkey: string): Promise<void> {
    try {
      const botAccount = new anchor.web3.PublicKey(botAccountPubkey)
  ...
  private startMonitoring(): void {
    ...
    console.log('🌐 RPC URL:', this.provider.connection.rpcEndpoint)
...
if (args.length > 0) {
  ...
  args.forEach(async (botAccount, i) => {
    try {
      new anchor.web3.PublicKey(botAccount) // Validate
